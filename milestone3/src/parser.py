...
def p_allocation_expression1(p): 
    ...
    tmp1 = getnewvar("int")
    p[0].place=getnewvar(p[0].data["type"])
    p[0].code = p[6].code + [quad("eq",[tmp1,str(get_size(tpe)), p[6].place],tmp1 + " = " + str(get_size(tpe)) + "*" + p[6].place), quad("PushParam",[tmp1,"",""],"PushParam " + tmp1)  , quad("Scall",["alloc",p[0].place],p[0].place + " = Scall alloc")]
